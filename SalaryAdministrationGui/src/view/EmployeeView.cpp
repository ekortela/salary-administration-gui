...
void EmployeeView::updateEmployeeList(vector<Employee *> model) {
    qDebug() << "Updating employeeList. Model element count: " << model.size();
    employeeList.clear();
    m_treeWidget->clear();
    unsigned int i = 0;
    for (vector<Employee*>::iterator it = model.begin(); it != model.end(); ++it) {
        ...
        qDebug() << "employeeList appended with " << i;
    }
}
...
void EmployeeView::handleSaveButtonClick() {
    ...
    popBox("Save button was clicked!");
    ...
    if (typ != employee_types::UNKNOWN) {
        bool status = observer->handleEventAddEmployee(typ,
                              firstName, lastName, ssn,
                              monthlySalary, hourlySalary, hoursDone, bonus, outcomeClaim);


        if (!status) {
            popBox("Unable to add employee!!");
        }
